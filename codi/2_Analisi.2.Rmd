...
header-includes:
  ...
  \pretitle{
    \begin{center}
    \vspace{-2cm} 
    \includegraphics[width=5cm]{CENTER LOGO.png}
    \hfill
    \text{}
    \\[\bigskipamount]
  }
...
<script>
   $(document).ready(function() {
     ...
     $head.prepend('<img src=\"../codi/CENTER LOGO.png\" style=\"float: left:1;width: 185px;\"/>')
   });
</script>
...
T1_kable <- dades %>% head() %>% 
  ...
  bind_cols(dades %>% head()) %>% 
  kableExtra::kable(caption = "Mostra de dades") %>% 
  kableExtra::kable_classic_2()

dades %>% head() %>% flextable::flextable()
...
generar_informe_word(path_arxiu="2_Analisi.Rmd" ,path_informe=here::here("outputs","informe_ray.docx"))
