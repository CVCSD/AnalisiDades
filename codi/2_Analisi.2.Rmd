...
```{r  Descriptiu_exploratori2, include=TRUE , fig.pos="H"}
...
```{r analisis_pergrups, eval=FALSE}
if(output_format=="pdf"){ 
  T1_kable <- dades %>% head() %>% 
    bind_cols(dades %>% head()) %>% 
    bind_cols(dades %>% head()) %>% 
    #booktabs millora el format de la  taula
    kableExtra::kable(caption = "Mostra de dades", format = "latex", booktabs = TRUE) %>% 
    ...
    bind_cols(dades %>% head()) %>% 
    #booktabs millora el format de la  taula
    kableExtra::kable(caption = "Mostra de dades", booktabs = TRUE) %>% 
    kableExtra::kable_classic_2()
}
# surt be la taula pero no es un kable
#T1_kable <- dades %>% head() %>% 
#  bind_cols(dades %>% head()) %>% 
#  bind_cols(dades %>% head()) %>% flextable::flextable() %>% set_caption(caption = "Mostra de dades", style = "Table Caption")



# Teula mal impresa quan es fa un render
# T1_kable <- dades %>% head() %>% 
#   bind_cols(dades %>% head()) %>% 
#   bind_cols(dades %>% head()) %>% 
#   kableExtra::kable(caption = "Mostra de dades", format = "latex") %>% 
#   kableExtra::kable_classic_2()
...
```{r analisis_pergrups_prova}
while(output_format=="pdf"){
  T1_kable <- dades %>% head() %>% 
    bind_cols(dades %>% head()) %>% 
    bind_cols(dades %>% head()) %>% 
    #booktabs millora el format de la  taula
