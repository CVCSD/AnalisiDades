...
output_format
output_format <- ifelse(output_format != "pdf" | output_format!= "html", "markdown", output_format)
```
...
# Word no treballa bé amb kable aixi que en fem flextable per a document word
...
T1_kable_dividida <-  if(output_format=="html") {dd_T1 %>% 
    #booktabs millora el format de la  taula
    kableExtra::kable(caption = "Mostra de dades", format = output_format, booktabs = TRUE) %>% 
    kableExtra::kable_classic_2() %>%
    # Hold_position evita que la taula aparegui a un altre
    kable_styling(latex_options = c("striped", "hold_position"))
} else if(output_format=="latex"){dividir_taula(dd_T1, n_columnes = 5) %>% 
    ...
    dividir_taula(dd_T1, n_columnes = 5) %>% lapply(function(bloc) {
      bloc %>% 
      flextable() %>%
        autofit() %>%
        width(width = 1.3)
